...
lineaInicialParaElSemestre = 0 #Esto se cambia cada semestre, antes decia 217 y es la linea donde empieza a leer, ahora dice 284

def system(s):
        """Llamado al comando system"""
        os.system(s)

def initialize(cur, grupos):
        """Se crean las carpetas donde se guardarán los trabajos"""
        system("mkdir "+trabajos)
        system("mkdir "+trabajos+"/"+cur)
        for i in range( len(grupos)) :
                system("mkdir "+trabajos+"/"+cur+"/"+grupos[i])
...
def processCSV():
        ...
        archivo.close()
        usuarios = dict()
        for linea in lines[int(lineaInicialParaElSemestre):]:
                line = linea.split(",")
                curso = line[4]
                grupo = line[5]
                codigo = line[1]
                codigoPareja = line[6].replace("\"", "")
                nombre = line[2]
                usuario = line[3].replace("\r","").replace("\"", "").replace("\n","").replace(" ","")
                usuarios[usuario] = (curso, grupo, codigo, codigoPareja, nombre) #Este es el formato del diccionario
...
def download(usuarios):
        ...
        for elUsuario, valores in usuarios.items():
                ...
                for usuario in [elUsuario]: # Se procesa solo para el dueño del repositorio                     
